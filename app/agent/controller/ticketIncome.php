<?php
class agent_ctl_ticketIncome extends agent_controller
{
		/**
	 	 * 异步获取数据  图表用
	 	 * @param null
		...
		public function ajaxOrder()
		{
			...
			{
				//计算昨天的日期
				$postData['starttime']=date("Y-m-d",strtotime("-1 day"));
				$postData['endtime']=date("y-m-d",time()); 
				
			}else if($type=="today")
			{
				$postData['starttime']=date("y-m-d",time());
				$postData['endtime']= time();
				
			}
			else if($type == "beforday")
			{
				$postData['starttime']=date("Y-m-d",strtotime("-2 day"));
				$postData['endtime']=date("Y-m-d",strtotime("-1 day"));
				
			}else if($type=="week")
			{
				$postData['starttime']=date("Y-m-d",strtotime("-7 day"));
				$postData['endtime']=time();
			}else if($type == "month")
			{
				$postData['starttime']=date("Y-m-d",strtotime("-30 day"));
				$postData['endtime']=time();
			}else if($type=="all")
			{
				$postData['starttime'] ="1970-01-01";
				$postData['endtime']= date("Y-m-d",time());
			}

			$all = $this->__getParams('graphall',$postData,'trade');
			$datas = app::get('agent')->rpcCall('agent.data.get',$all,'seller');
			$datas['label']="2015/08/25";
		...
		public function index()
		{
			...
			//详细交易数据
	 			 ...
	 			 if($agent_grade=="省")
	 			 {
	 			 	...
	 			 	$reTotal= app::get('syshdtj')->database()->executeQuery("SELECT count(*) as tol FROM  `syshdtj_order` where   `payed_time`  between ? AND  ? ",[$TimeStar,$TimeEnd])->fetchAll();
	 			 	$total=$reTotal[0]['tol'];
			...
			//近30天	
	 			 ...
	 			 $monthTimeEnd=time();
	 			 $pagedata['month'][]= app::get('syshdtj')->database()->executeQuery("SELECT sum(payment) as addOrderMoney,sum(num) as addOrderNum,avg(price) as avgPrice FROM  `syshdtj_order` where `order_from` in('". $order_from."')  and (  `payed_time`  between ? AND  ?)",[$monthTimeStar,$monthTimeEnd])->fetchAll();
	 			 ...
	 			 /*分页*/
	 			  ...
	 			  $filter['pages'] = time();
	 			  $pagers = array(
	 			  	'link'=>url::action('agent_ctl_ticketIncome@index',$filter),
	 			...
	 			*/
	 			...
	 			/*if($agent_grade == "县")//如果是县代理
	 			{
	 				//县代理的总月结报表
	 					...
	 					//$data['month']=app::get('syshdtj')->database()->executeQuery("SELECT sum(szx_price) as szx_price ,sum(num) as num FROM  `syshdtj_order` where `szx_id` in(".$inCod.")" ." and ` payed_time ` between ? and ?",[ 1 ,2])->fetchAll();
	 					$TimeStar = strtotime('15-7-30');
	 					$TimeEnd = strtotime('15-8-30'); 
	 					$data['month']= app::get('syshdtj')->database()->executeQuery("SELECT sum(szx_price) as szx_price ,sum(num) as num FROM  `syshdtj_order` where  `szx_id` in(?)  and `payed_time`  between ? AND  ? ",[$inCod,$TimeStar,$TimeEnd])->fetchAll();
	 		

	 			}else if($agent_grade == "市")//如果是市代理
	 			{

	 			}else if($agent_grade == "省")//如果是省代理
	 			{
	 				foreach ($data['code'] as $key => $value) {
	 					if(substr($value, 4,2)!="00")//如果省代理代理了特定县，则返回order表，此区域的，演唱会的，已结算的订单
	 					{
	 						$pagedata[]= app::get('syshdtj')->database()->executeQuery("SELECT  * FROM  `syshdtj_order`  ")->fetchAll();
	 						
	 					}else if(substr($value, 2,4)=="0000")//如果省代理代理了特定市
	 					{
	 			...
	 			//业务报表分页
            		...
            		'token'=>time(),
	 				...
	 				return $this->page('agent/ticketIncome.html', $pagedata);
	 			}
