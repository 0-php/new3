...
class topc_ctl_item extends topc_controller {
    ...
    private function fenxiao($itemId){ //公众店铺业务逻辑
        $params['shopId'] = intval(input::get('shopId'));  //是否传来有分销店铺ID
        if(!empty($params['shopId'])) { //判断是否有传进来分销商品店铺id
            $shopid = app::get('topd')->model('dp')->getRow("*", array('user_id' => $params['shopId']));
            if (!empty($shopid['user_id'])) {
                $shopgoods = app::get('topd')->model('goods')->getRow("item_id", array('item_id' => $itemId,'user_id' => $params['shopId'], 'goods_zt' => 'onsale'));
                if(!empty($shopgoods['item_id'])){
                    $pagedata['shopId'] = $shopid['user_id'];
                    $pagedata['shopdp'] = $shopid;
                    $fenxiao_clk = explode('|',$shopid['fenxiao_clk']);
                    if(strtotime(date('Y-m-d')) >= $fenxiao_clk[0]){
                        $paclk = time()."|1|".($fenxiao_clk[2]+1);
                    }else{
                        $paclk = $fenxiao_clk[0]."|".($fenxiao_clk[1]+1)."|".($fenxiao_clk[2]+1);
                    }
                    if(!empty($_COOKIE['topublic'])){
                        $topublic = unserialize($_COOKIE['topublic']);
                    }
                    $topublic[$itemId] = $shopid['user_id'];
                    setcookie('topublic',serialize($topublic),time()+86400,'/'); //1天内点击购买有效
                    app::get('topd')->model('dp')->update(array('fenxiao_clk' => $paclk),array('user_id' => $shopid['user_id']));
                }
            }
        }
    }
