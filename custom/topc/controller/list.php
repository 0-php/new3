...
class topc_ctl_list extends topc_controller {
    ...
    public function fxindex(){
        ...
        if(empty($shopid['user_id'])){ //如果分销商不存在
        ...
        //公众店铺筛选
        $params['initem'] = app::get('topd')->model('goods')->getList("item_id", array('user_id' => $params['shopId'], 'goods_zt' => 'onsale'));
        $params['item_id'] =  implode(",", array_column($params['initem'],'item_id'));
        $params['item_id'] = empty($params['item_id']) ? 1 : $params['item_id']; //商品id为1就为空
        ...
        $pagedata['shopId'] = $params['shopId'];
        $pagedata['fenxiao'] = $shopid;

        //输出模板
        $shopid['fenxiao_cidimg'] = empty($shopid['fenxiao_cidimg'])?0:$shopid['fenxiao_cidimg'];
        return $this->page('topc/list/fxindex'.$shopid['fenxiao_cidimg'].'.html', $pagedata);
    }
