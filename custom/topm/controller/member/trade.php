...
class topm_ctl_member_trade extends topm_ctl_member{
    ...
    private function __getTrade($postdata)
    {
        ...
        $params['fields'] = 'hdvip_fee,tid,shop_id,user_id,status,payment,total_fee,itemnum,post_fee,payed_fee,receiver_name,created_time,receiver_mobile,discount_fee,need_invoice,adjust_fee,order.title,order.price,order.num,order.pic_path,order.tid,order.oid,order.aftersales_status,buyer_rate,order.complaints_status,order.item_id,order.shop_id,order.status,activity,pay_type';
