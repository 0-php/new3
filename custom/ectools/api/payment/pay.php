...
class ectools_api_payment_pay{
    ...
    public function topddoPay($params)  //新添加公众店铺开店处理 Ric
    {
        if(!$params['platform'])
        {
            $params['platform'] = "pc";
        }
        $objMdlPayments = app::get('topd')->model('dp');
        $paymentBilla = $objMdlPayments->getRow('*',array('user_id'=>$params['user_id']));
        if($paymentBilla['fenxiao_zt'] != 'active')
        {
            throw new Exception('该订单已经支付');
            $paymentBill['status'] = 'succ';
        }else{
            $paymentBill['status'] = 'ready';
        }
        $paymentBill['pay_app_id'] = $params['pay_app_id'];
	$paymentBill['item_title'] = $params['item_title'][0];
        $paymentBill['payment_id'] = $paymentBilla['payment_id'];
        $paymentBill['money'] = $params['money'];
        $paymentBill['pay_type'] = 'online';
        $paymentBill['currency'] = '';
        $paymentBill['cur_money'] = $params['money'];
        $objPayment = kernel::single('ectools_pay');
        $result = $objPayment->generate($paymentBill);
        if(!$result)
        {
